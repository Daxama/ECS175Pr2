...
function parseCamera( entry )
{
    ...
    let fov = entry.fov//vertical field of view, currently in degrees
    fov = deg2rad(fov)
    ...
    {
        camera = new PerspectiveCamera(position, lookat, up, fov, aspect, near, far)
        // TODO create a perspective camera here
        
    }
    else if ( entry.type == "orthographic" )
    {
        camera = new OrthographicCamera(rect, near, far)
